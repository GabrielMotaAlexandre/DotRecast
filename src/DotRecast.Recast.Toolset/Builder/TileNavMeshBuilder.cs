...
namespace DotRecast.Recast.Toolset.Builder
{
    public class TileNavMeshBuilder
    {
        ...
        public static List<RcBuilderResult> BuildRecastResult(IInputGeomProvider geom,
            int tileSize,
            RcPartition partitionType,
            float cellSize, float cellHeight,
            float agentMaxSlope, float agentHeight, float agentRadius, float agentMaxClimb,
        ...
        public static DtNavMesh BuildNavMesh(IInputGeomProvider geom, List<DtMeshData> meshData, float cellSize, int tileSize, int vertsPerPoly)
        {
            ...
            DtNavMesh navMesh = new(navMeshParams, vertsPerPoly);
        ...
        {
            RcCommons.CalcGridSize(geom.GetMeshBoundsMin().AsVector2XZ(), geom.GetMeshBoundsMax().AsVector2XZ(), cellSize, out var gw, out var gh);
